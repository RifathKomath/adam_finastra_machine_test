...
import '../../app/model/onboarding/login_response.dart';
...
class SharedPref {
  ...
  getUserData() async {
    ...
    getLoginedUserData() async {
    if (sharedPref == null) await init();
    final String? loginedUserDataJson = sharedPref?.getString("userlogdata");

    if (loginedUserDataJson == null ||
        loginedUserDataJson.isEmpty ||
        loginedUserDataJson == "null") {
      return null;
    }

    try {
      final decoded = jsonDecode(loginedUserDataJson);

      if (decoded == null || decoded is! Map<String, dynamic>) {
        return null;
      }

      userLoginedData = UserLoginedData.fromJson(decoded);
      return userLoginedData;
    } catch (e) {
      print("Error parsing login user data: $e");
      return null;
    }
  }
